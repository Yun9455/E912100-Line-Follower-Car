...
// 幾何控制參數：針對 18 公分前懸(感測器到輪軸距離)進行最佳化
const float Ld = 140.0;          // 前視距離 (Lookahead Distance)，作為 Pure Pursuit 的假想目標點距離
...
const int SPEED_SEARCH = 95;     // 盲區尋線時，執行原地樞軸轉向(Pivot Turn)的速度
// ESP32 PWM 硬體定時器設定
